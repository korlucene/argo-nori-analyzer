...
public final class KoreanTokenizer extends Tokenizer {
   ...
   */
  private int computePenalty(POS.Tag posTag, int endPos) throws IOException {
  	 
  	if(posTag == POS.Tag.ETN || posTag == POS.Tag.EP)
  		return 0;
  	
  	char tag = posTag.name().charAt(0);
  	
		if(tag=='E' || tag == 'J') {
		  char nextCh = (char)buffer.get(endPos);
		  int characterType = Character.getType(nextCh);
		  
		  if(characterType!=Character.SPACE_SEPARATOR)
		  	return 4100;
		}
  ...
  private void add(Dictionary dict, Position fromPosData, int wordPos, int endPos, int wordID, Type type) throws IOException {
    ...
    for(int idx=0;idx<fromPosData.count;idx++) {
      ...
      final int cost = fromPosData.costs[idx] + costs.get(fromPosData.lastRightID[idx], leftID) + computePenalty(leftPOS, endPos);
      if (VERBOSE) {
        System.out.println("      fromIDX=" + idx + ": cost=" + cost + " (prevCost=" + fromPosData.costs[idx] + " wordCost=" + wordCost + " bgCost=" + costs.get(fromPosData.lastRightID[idx], leftID) +
            " spacePenalty=" + computePenalty(leftPOS, endPos) + ") leftID=" + leftID + " leftPOS=" + leftPOS.name() + ")");
      }
      if (cost < leastCost) {
        leastCost = cost;
        leastIDX = idx;
